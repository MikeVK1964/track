...
void BaseView::CorNTras(int num_pos)
{
...
void BaseView::FillElList(int i)
{
 QString str;
...
void BaseView::AddTras(float B, float D)
{
 DocTras* m_pDoc;
 m_pDoc=GetDocument();
...
void BaseView::SetCurSel(int cur_sel)
{
...
// h - по умолчанию
int BaseView::GetDefH()
{
 return pright1->ui->sB_H->value();
}
unsigned int BaseView::GetDefA()
{
 return pright1->ui->lE_A->text().toUInt();
}
unsigned int BaseView::GetDefV()
{
 return pright1->ui->lE_V->text().toUInt();
}
void BaseView::BaseConstruct()
{
        QSplitter* pleft;
